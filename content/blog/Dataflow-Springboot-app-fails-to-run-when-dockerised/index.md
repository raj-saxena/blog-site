...
We needed a DataWarehouse for analysis and reporting and the setup made it a bit tricky as we didn't want to use an external ETL service or have long living database credentials.
...
> 1) Connect to Vault to generate read-only credential for each database.
> 2) Read the required tables and data. Apply any transformations like type conversion, anonymisation or pseudonymization.
...
Dataflow is built on [Apache Beam](https://beam.apache.org/)'s programming and execution model that expects you to define a `Pipeline` that connects the `source`, `transformations` to apply and `sink`. It then executes the `Pipeline` job over a auto-scaling fleet of compute instances.
...
The microservice would need access permissions to the following Google services -  BigQuery, Google storage (used to store temporary files), Dataflow, Compute instances. So, I created a dedicated serviceAccount with the necessary permissions. [Service accounts](https://cloud.google.com/iam/docs/service-accounts) are a special kind of account used by an application or a virtual machine (VM) instance, not a person.

I built the service and tested multiple runs which were all __successful when I ran the service from the IDE__ or when I did `./gradlew run` from command line.
...
Assuming that `./gradlew build` has run before successfully, it copies the fat-jar inside the image and then runs it when container is started.  
...
There would be a log of this printed every 5-6 minute for up to an hour at which point the job would fail.
...
The logs hinted at a possible permissions issue with the serviceAccount. However, it did not make total sense as the execution was successful from within the IDE or when ran with Gradle. I made sure that there were no other GoogleCredentials present in my local environment and the serviceAccount was the only credential injected as environment variable. In fact, all the environment variables were the same except the part of containerization. 

I enabled `debug` logs but that didn't gave any useful insight either. The job would get stuck with the same error. Google console displayed failed job with the message:
...
I even opened a Google support case from our account but didn't get a helpful answer. Mostly, because they couldn't find anything wrong on there side.
...
### The Experiment
Working on a tight deadline, I decided to dockerise my whole repo and run `/gradlew run` from inside it. 
Creating a fat-jar and dockerising it 
...
```
It failed like the docker version and at the same point.
> Checking permissions granted to controller Service Account. 

____

It had been a wild journey. I decided till I figure what's going on, I am going to deploy what's working.
...
I don't know why a fat-jar doesn't work here. This is the first time I faced this and I definitely want to understand better. Perhaps, someone else in the community knows why and share it.
...
### Conclusion
* Computers and modern software is complex and hard. In the entire feat of integrating a mix of complex technologies like Vault, Kubernetes, BigQuery, Dataflow and the service logic itself, this little thing caused me the most pain.
